...
The presence of multi-year buyers means that an extra step is needed to produce customer trends. This is accomplished in salic by calling 2 functions in sequence (`rank_sale` & `make_history`).
...
The R3 measure is used to group license holders based on how recently they bought licenses. For the national/regional dashboards, the recruited group is of particular interest.

- Retained: (held a license last year)
    + R3 = 1 (carried over a license from last year, i.e., didn't purchase this year)
    + R3 = 2 (didn't carry over a license)
...
Some initial preparation is needed to produce the summaries.
...
# view the first 3 rows of each segment
for (i in segments) {
    head(part[[i]], 3) %>% print()
