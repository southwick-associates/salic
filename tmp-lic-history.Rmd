...
# check R3 coding
filter(lic_history, !(year %in% yrs[1:5])) %>%
    count(has_priv, R3, lag_duration_run) %>% 
    tidyr::spread(R3, n)
```

### Identify Lapse - START HERE
- maybe just include a random sample test below
- (might be worth doing for R3 as well)
```{r}
...
# take a random sample (or maybe do lag(has_priv)) ...
lic_history
```
