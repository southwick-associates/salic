...
```{r}
f <- "~/Data/MO_Dashboard/ProductInfo.csv" # path to raw data file
...
```{r}
f <- "~/Data/MO_Dashboard/MDC_Data/tblCustomer.txt"
read_lines(f, n_max = 5)
...
```{r}
f <- "~/Data/MO_Dashboard/MDC_Data/tblPrivilegeSales.txt"
read_lines(f, n_max = 5)
read_csv(f, n_max = 5) %>% glimpse()
```
...
```{r}
dir <- "~/Data/MO_Dashboard/" # directory for sqlite database
f <- file.path(dir, "raw-tmp.sqlite3")
