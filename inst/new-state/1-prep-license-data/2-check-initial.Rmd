...
```{r}
db <- src_sqlite("E:/SA/Data-Dashboards/Data/IA/raw-2015.sqlite3")
src_tbls(db)
```

**Guidelines**

- only a subset of sales columns are needed:
    + at least: cust_id, lic_id, year
...
```{r}
sale <- tbl(db, "sale") %>% 
    select(year = TXNL_YEAR, cust_id = TXNL_CUST_ID, lic_id = TXNL_LICN_TYPE) %>%
    collect()
glimpse(sale)
```
...
## START HERE - Check by License Type
...
```{r}
lic <- tbl(db, "lic") %>% 
    select(lic_id = LTYPE_LICN_TYPE, lic_id_alt = LTYPE_ID, year = LTYPE_YEAR,
           description = LTYPE_DESC) %>%
    collect()
glimpse(lic)
```
...
- https://wsfrprograms.fws.gov/Subpages/LicenseInfo/Hunting.htm
    + Note that USFWS track **paid** license holders, so excluding free transactions can sometimes reconcile perceived differences between USFWS-based and data-based counts
...
```{r}
all_sale <- select(lic, lic_id, description, type) %>% 
    full_join(sale)
count(all_sale, type, lic_id, description, year) %>% 
    spread(year, n, fill = 0) %>% 
    write.csv(file = "../../out/1-prep-license-data/sales-by-lic-year.csv", 
              row.names = FALSE)
```
