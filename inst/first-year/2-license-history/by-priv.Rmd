...
# MO specific, setting all duration to 1, so lifetime license are only counted in first year
lic <- mutate(lic, duration = 1)
DT::datatable(lic)
```
...
```{r}
# MO-specific residency recoding
# daily small game are actually nonresident only
if (params$priv_nm == "daily_small_game") sale$res <- 0
if (params$priv_nm == "sg_and_fish") sale$res <- 1
```
