...
params:
    ...
    out_dir: "../../data/6-dashboard-results"  # directory for output RData (may vary by state)
---
...
### County Recoding - MO specific

- Combine "St Lous City" & "St Louis" together

```{r}
priv <- priv %>% mutate(
    county_old = county,
    county = stringr::str_replace(county, "St. Louis city", "St. Louis")
)
filter(priv, county == "St. Louis") %>% count(county_old, county)
# count(priv, county_old, county) %>% DT::datatable()
priv <- select(priv, -county_old)
```

```{r}
pop_county <- pop_county %>% mutate(
    county_old = county,
    county = stringr::str_replace(county, "St. Louis city", "St. Louis")
)
filter(pop_county, county == "St. Louis") %>% count(county_old, county)
# count(pop_county, county_old, county) %>% DT::datatable()
pop_county <- group_by(pop_county, year, county, sex, age) %>%
    summarise(pop = sum(pop)) %>%
    ungroup()

length(unique(pop_county$county)) == 114
length(unique(pop_county$county)) == length(setdiff(unique(priv$county), NA))
```
